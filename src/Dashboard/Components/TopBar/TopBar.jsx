...
//       // Hide already read notifications initially
//       const readNotifications = fetchedNotifications
//         .filter((notif) =>
//           notif.readBy.includes(localStorage.getItem("userid"))
//         )
//         .map((notif) => notif._id);
...
//             (n) => !n.readBy.includes(localStorage.getItem("userid"))
//           ).length > 0 && (
...
//                 ).map((notif) => {
//                   const isRead = notif.readBy.includes(
...
//                         hiddenNotifications.includes(notif._id) ? "d-none" : ""
//                       }`}
//                     >
//                       <div className="d-f">
...
const TopBar = ({ onSearch }) => {
    ...
    const getUserData = async () => {
      try {
        ...
        if (userId) {
          ...
          const userData = response.data.user;
          setUserName(userData.name || "User");
          setUserProfilePic(userData.profilePic.startsWith("data:image/") ?
                      userData.profilePic : `data:image/jpeg;base64,${userData.profilePic}|| defaultProfileSVG`);
  ...
  const markAsRead = async (notificationId) => {
    try {
      ...
      // Immediately add "read" class to the button
      setReadButtons((prev) => ({ ...prev, [notificationId]: true }));
